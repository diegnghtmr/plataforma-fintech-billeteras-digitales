...
class WalletControllerTest {
    ...
    @Test
    void createWallet_S6_duplicateCode_returns409() throws Exception {
        when(createWalletUseCase.execute(eq("USR001"), eq("WALLET-001"), anyString(), anyString()))
        ...
        mockMvc.perform(post("/users/USR001/wallets")
                        ...
                        .content(objectMapper.writeValueAsString(
                                Map.of("code", "WALLET-001", "name", "Dup", "type", "SAVINGS"))))
    ...
    void listWallets_S7_twoWallets_returns200WithList() throws Exception {
        List<Billetera> wallets = List.of(
                new Billetera("W1", "Ahorros", "SAVINGS", "USR001", 500.0, true, FIXED_NOW, 0),
                new Billetera("W2", "Gastos", "DAILY", "USR001", 300.0, true, FIXED_NOW, 0)
        );
        List<WalletResponseDto> dtos = List.of(
                new WalletResponseDto("W1", "Ahorros", "SAVINGS", "USR001", 500.0, true, FIXED_NOW.toString(), 0),
                new WalletResponseDto("W2", "Gastos", "DAILY", "USR001", 300.0, true, FIXED_NOW.toString(), 0)
        );
    ...
    @Test
    void createWallet_happyPath_returns201() throws Exception {
        Billetera billetera = new Billetera("W001", "Ahorros", "SAVINGS", "USR001", 0.0, true, FIXED_NOW, 0);
        WalletResponseDto dto = new WalletResponseDto("W001", "Ahorros", "SAVINGS", "USR001", 0.0, true, FIXED_NOW.toString(), 0);

        when(createWalletUseCase.execute("USR001", "W001", "Ahorros", "SAVINGS")).thenReturn(billetera);
        when(walletMapper.toDto(billetera)).thenReturn(dto);

        mockMvc.perform(post("/users/USR001/wallets")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(
                                Map.of("code", "W001", "name", "Ahorros", "type", "SAVINGS"))))
