...
class FullFlowIntegrationTest {
    ...
    void executesFullFlow() {
        ...
        // -----------------------------------------------------------------------
        // S2 — Create wallet for generated user A (code auto-generated by backend)
        // -----------------------------------------------------------------------
        ...
        String walletA1 = s2.getBody().code();
        assertThat(walletA1).matches("WAL\\d{3,}");
        assertThat(s2.getBody().balance()).isEqualTo(0.0);

        // -----------------------------------------------------------------------
        // S3 — Recharge walletA1 with 20000; capture txIdRecharge
        // -----------------------------------------------------------------------
        ...
        ResponseEntity<TxR> s3 = rest.exchange(
                base + "/users/" + userIdA + "/wallets/" + walletA1 + "/recharge",
                HttpMethod.POST,
        ...
        WalletR w01AfterRecharge = walletsAfterRecharge.getBody().stream()
                .filter(w -> walletA1.equals(w.code()))
                .findFirst().orElseThrow();
        assertThat(w01AfterRecharge.balance()).isEqualTo(20000.0);

        // -----------------------------------------------------------------------
        // S4 — Withdraw 1000 from walletA1; capture txIdWithdraw
        // -----------------------------------------------------------------------
        ...
        ResponseEntity<TxR> s4 = rest.exchange(
                base + "/users/" + userIdA + "/wallets/" + walletA1 + "/withdraw",
                HttpMethod.POST,
        ...
        // -----------------------------------------------------------------------
        // S5 — Create second wallet for user A and perform internal transfer 500
        // -----------------------------------------------------------------------
        ...
        String walletA2 = s5a.getBody().code();
        assertThat(walletA2).matches("WAL\\d{3,}");

        Map<String, Object> internalTransferBody = Map.of(
                "sourceWalletId", walletA1,
                "targetWalletId", walletA2,
                "amount", 500.0,
        ...
        // Verify balances: walletA1=18500, walletA2=500
        ...
        WalletR w02AfterInternal = walletsAfterInternal.getBody().stream()
                .filter(w -> walletA2.equals(w.code()))
                .findFirst().orElseThrow();
        ...
        //       and balance walletA1 = 18500 + 1000 = 19500
        // -----------------------------------------------------------------------
        ...
        // -----------------------------------------------------------------------
        // S8 — Points and loyalty level for user A
        // -----------------------------------------------------------------------
        ...
        // -----------------------------------------------------------------------
        // S9 — Create USR-B + wallet B; perform external transfer 15000
        ...
        // Create wallet for user B (code auto-generated)
        ...
        assertThat(s9CreateWalletB.getBody()).isNotNull();
        String walletB1 = s9CreateWalletB.getBody().code();
        assertThat(walletB1).matches("WAL\\d{3,}");

        // Perform external transfer 15000 (walletA1 has 19500 — sufficient)
        Map<String, Object> externalTransferBody = Map.of(
                "sourceUserId", userIdA,
                "sourceWalletId", walletA1,
                "targetUserId", userIdB,
                "targetWalletId", walletB1,
                "amount", 15000.0,
